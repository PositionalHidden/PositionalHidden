...
class MptAttention(nn.Module):
    ...
    def __init__(self, config: MptConfig,layer_idx):
        super().__init__()
        self.config = config
    ...
    def attention_forward(self,query_states, key_states, value_states, attention_mask,position_bias):
    ...
    def _init_hidden_states_scale(self,hidden_states):
        ...
        hidden_state_set_layers = hidden_scale_config["layers"]
        hidden_state_set_dims = hidden_scale_config["dims"]
        skip_first = hidden_scale_config["skip_first"]
        scale_foctor = hidden_scale_config["factor"]
        last_recompute_tokens=hidden_scale_config["last_recompute_tokens"]
        if hidden_scale_config['change_value']:
            warnings.warn("In MptAttention, change_value is not supported yet.")
            hidden_scale_config['change_value'] = False

        change_value = hidden_scale_config["change_value"]

        if self.layer_idx not in hidden_state_set_layers:
            hidden_states_scaled=hidden_states
        else:
            hidden_states_scaled = hidden_states.clone()
            hidden_states_scaled[:, skip_first:, hidden_state_set_dims] *= scale_foctor

        #calculate qkv
        mixed_qkv_scaled = self.Wqkv(hidden_states_scaled)
        query_states_scaled, key_states_scaled, value_states_scaled = mixed_qkv_scaled.chunk(3, dim=2)
        query_states_scaled = query_states_scaled.view(bsz, q_len, self.n_heads, self.head_dim).transpose(1, 2)
        key_states_scaled = key_states_scaled.view(bsz, q_len, self.n_heads, self.head_dim).transpose(1, 2)
        value_states_scaled = value_states_scaled.view(bsz, q_len, self.n_heads, self.head_dim).transpose(1, 2)

        value_states_scaled = value_states_scaled if change_value else None

        return query_states_scaled,key_states_scaled,value_states_scaled
    ...
    def forward(
        ...
        if seq_length==1:
            ...
            if past_key_value is not None:
                ...
                #创建一个新的past_key_value
                past_key_value = (key_states, value_states)

            context_states,attn_weights = self.attention_forward(query_states, key_states, value_states, attention_mask, position_bias)
            ...
            #创建一个新的past_key_value
            past_key_value = (key_states_scaled, value_states_scaled)

            #计算注意力
            context_states,attn_weights = self.attention_forward(query_states, key_states, value_states, attention_mask, position_bias)
            #最后几个token重新计算注意力
            last_recompute_tokens=query_states_scaled.shape[2]
            query_states_scaled_last = query_states_scaled[:,:, -last_recompute_tokens:,:]
            attention_mask_last = attention_mask[:,:, -last_recompute_tokens:,:]
            context_states_last ,attn_weights_last= self.attention_forward(query_states_scaled_last, key_states_scaled, value_states_scaled, attention_mask_last, position_bias)
            #替换最后一行
            context_states[:, -last_recompute_tokens:,:] = context_states_last
            attn_weights[:,:, -last_recompute_tokens:,:] = attn_weights_last
...
class MptBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_set_layers = getattr(self.config, "hidden_set_layers", {})
        hidden_set_values = getattr(self.config, "hidden_set_values", 0)
        hidden_set_from = getattr(self.config, "hidden_set_from", 0)
        hidden_set_to = getattr(self.config, "hidden_set_to", hidden_states.shape[1])
        hidden_set_pos = getattr(self.config, "hidden_set_pos", "qkv")
        if hidden_set_pos=="norm1":
            if self.layer_idx in hidden_set_layers:
                if isinstance(hidden_set_values, str) and "/" in hidden_set_values:
                    scale_factor = float(hidden_set_values.split("/")[1])
                    hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]] /= scale_factor
                elif isinstance(hidden_set_values, str) and hidden_set_values=="mean":
                    # 将指定维度设为hidden_set_values
                    hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]] = hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]].mean()
                elif isinstance(hidden_set_values, str) and hidden_set_values=="min":
                    # 将指定维度设为hidden_set_values
                    hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]] = hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]].min()
                elif isinstance(hidden_set_values, str) and hidden_set_values=="max":
                    # 将指定维度设为hidden_set_values
                    hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]] = hidden_states[:, 20:hidden_set_to, hidden_set_layers[self.layer_idx]].max()
                else:
                    # 将指定维度设为hidden_set_values
                    hidden_states[:, hidden_set_from:hidden_set_to, hidden_set_layers[self.layer_idx]] = hidden_set_values
...
class MptModel(MptPreTrainedModel):
    def __init__(self, config: MptConfig):
        ...
        # Transformer blocks
        self.blocks = nn.ModuleList([MptBlock(config,layer_idx) for layer_idx in range(config.n_layers)])
