...
class GemmaAttention(nn.Module):
    ...
    def _init_hidden_states_scale(self,hidden_states):
        ...
        if change_value:
            value_states_scaled = self.v_proj(hidden_states_scaled)
            value_states_scaled = value_states_scaled.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        else:
            value_states_scaled = None

        return query_states_scaled,key_states_scaled,value_states_scaled,last_recompute_tokens
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states_scaled,key_states_scaled,value_states_scaled,last_recompute_tokens = self._init_hidden_states_scale(hidden_states)

        if q_len == 1 and use_cache:
            query_states =query_states_scaled
            key_states = key_states_scaled
            if value_states_scaled is None:
                #because value has not been computed, so we need to compute it
            ...
            cos, sin = self.rotary_emb(value_states, position_ids, seq_len=None)
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, None)
            _, key_states_scaled = apply_rotary_pos_emb(key_states_scaled, key_states_scaled, cos, sin, None)
            #apply rotary position embedding to query_states which is scaled
            cos_,sin_=self.rotary_emb(query_states_scaled,position_ids[:, -last_recompute_tokens:],seq_len=None)
            query_states_scaled,_ = apply_rotary_pos_emb(query_states_scaled, query_states_scaled, cos_, sin_, None)

            if past_key_value is not None:
                # sin and cos are specific to RoPE models; position_ids needed for the static cache
                cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                past_key_value.update(key_states_scaled, value_states_scaled, self.layer_idx, cache_kwargs)
            ...
            if attention_mask is not None:  # no matter the length, we just slice it
                if cache_position is not None:
                    causal_mask = attention_mask[:, :, cache_position, : key_states.shape[-2]]
                    # crop attention_mask
                    attention_mask_last = causal_mask[:, :, -last_recompute_tokens:, :]
                else:
                    causal_mask = attention_mask
                    # crop attention_mask
                    attention_mask_last = causal_mask[:, :, -last_recompute_tokens:, :]
            ...
            attn_output = attn_output.reshape(bsz, q_len, -1) #reshape to [bsz, q_len, hidden_size]
            ...
            if attention_mask_last is not None:
            ...
            attn_output_last = attn_output_last.reshape(bsz, last_recompute_tokens, -1)
            #替换最后一行
    ...
    def forward(
        ...
        **kwargs,
        ...
        if q_len == 1 and use_cache:
            ...
            cos, sin = self.rotary_emb(value_states, position_ids,seq_len=kv_seq_len)
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
            _, key_states_scaled = apply_rotary_pos_emb(key_states_scaled, key_states_scaled, cos, sin)
            ...
            #transpose qkv to shape [batch_size, sequence_length, num_heads, head_dim]
            query_states = query_states.transpose(1, 2)
            key_states = key_states.transpose(1, 2)
            value_states = value_states.transpose(1, 2)
            query_states_scaled = query_states_scaled.transpose(1, 2)
            key_states_scaled = key_states_scaled.transpose(1, 2)
            value_states_scaled = value_states_scaled.transpose(1, 2)
            ...
            # 对最后一行，使用key_states_set0作为key
            attention_mask_last = attention_mask[:,:, -last_recompute_tokens:,:] if attention_mask is not None else None
            attn_output_last = self._flash_attention_forward(
                query_states_scaled, key_states_scaled, value_states_scaled, attention_mask=attention_mask_last,
                query_length=last_recompute_tokens, dropout=dropout_rate
            )
            attn_output_last = attn_output_last.reshape(bsz, last_recompute_tokens, -1).contiguous()
            #替换最后一行
            attn_output[:,-last_recompute_tokens:,:] = attn_output_last
...
class GemmaSdpaAttention(GemmaAttention):
    ...
    def forward(
        ...
        logger.warning_once(
            "SdpAttention is not yet supported in Gemma. Please use `GemmaAttention` or `GemmaFlashAttention2` instead."
        )
